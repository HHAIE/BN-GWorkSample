...
class VideoPlayer:
    ...
    def show_all_playlists(self):
        ...
        if len(lists) == 0:
            ...
            for list_name in lists:
                if len(self.playlists[list_name]) == 1:
                    print(f"  {list_name} ({len(self.playlists[list_name])} video)")
                else:
                    print(f"  {list_name} ({len(self.playlists[list_name])} videos)")
    ...
    def show_playlist(self, playlist_name):
        ...
        if playlist_name.lower() not in map(str.lower, self.playlists.keys()):
            ...
            print("Would you like to create a new playlist with this name? yes/no")
            while True:
                command = input("> ")
